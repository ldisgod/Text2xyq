...
CHARACTER_TYPES_MAP: dict[str, list[str]] = {
    "宠物": [
        "猫", "狗", "仓鼠", "兔子", "鹦鹉", "乌龟", "狐狸", "松鼠", "金鱼", "蜥蜴",
        "熊猫", "小浣熊", "水獭", "柴犬", "布偶猫", "龙猫", "小鹿", "企鹅", "考拉", "刺猬",
    ],
    "人物": [
        "普通少女", "落魄少爷", "现代白领", "古代书生",
...
DEFAULT_TEMPLATES: dict[str, str] = {
    ...
    "outline_user": (
        ...
        "\n"
        "【要求】\n"
        "1. 首先为本剧起一个吸引人的剧名，用「剧名：XXX」的格式输出在第一行。\n"
        "2. 输出整体故事大纲：背景设定、主要人物、核心冲突、故事走向、结局方向。\n"
        "3. 大纲需支撑 ${episode_count} 集剧情，节奏紧凑，每集都有看点。\n"
        "4. 每集视频目标时长约 ${episode_duration_seconds} 秒，请据此控制信息密度。\n"
        "\n"
        "请直接输出大纲内容，不需要额外说明。"
    ),
    ...
    "shot_system": (
        ...
        "\n"
        "## 画面内容红线\n"
        "- 严禁出现血腥、恶心、暴力血溅等令人不适的画面描述\n"
        "- 冲突场景用动作暗示代替直接展示，如「一爪挥出，对方踉跄后退」而非血腥细节\n"
        "\n"
        "## 台词质量要求\n"
        "- 台词必须有信息量，推动剧情或展现角色性格，禁止无意义的感叹和废话\n"
        "- 严禁使用「呜……」「哦……」「啊……」「嗯……」等纯语气词充当台词\n"
        "- 即使是动物角色，说话也要像人一样清晰易懂，观众能从台词中获取信息\n"
        "- 每句台词都要有目的：或传递信息、或制造冲突、或揭示心理、或推动转折\n"
        "\n"
        "## 风格要求\n"
        "- 画面风格：${visual_style}\n"
        "- 画面比例：${aspect_ratio}\n"
...
def parse_drama_title(outline_text: str) -> str:
    """从大纲文本中提取剧名（首行「剧名：XXX」格式）。"""
    m = re.search(r'剧名[：:]\s*(.+)', outline_text)
    if m:
